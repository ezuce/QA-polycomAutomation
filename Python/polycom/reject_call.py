#!/usr/bin/env python
#
#	This script rejects an incoming call (call_reference)
#
#	# 	Get status: 
# curl --digest -k -X GET https://Test:Test@10.1.0.14/polling/callstateHandler
#
# 	Answer call: 
# curl --digest -u Test:Test -d $'<PolycomIPPhone><Data priority="Critical">CallAction:Reject;nCallReference=0x411cbc80</Data></PolycomIPPhone>' --header "Content-Type: application/x-com-polycom-spipx" https://10.1.0.14/push
#
#
...
# Reject call from phone_ip_2 (line 202)

api_url = 'https://{}/push' .format(phone_ip)
s_headers = {'Content-Type': 'application/x-com-polycom-spipx'}
s_data = ' <PolycomIPPhone><Data priority="Critical">CallAction:Reject;nCallReference={}</Data></PolycomIPPhone> ' .format(values.text)
